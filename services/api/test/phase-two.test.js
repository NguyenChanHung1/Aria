...
function fixture() {
  ...
  const writes = [];
  const artifacts = {
    getArtifact: async () => ({ payload: { sourceType: [{ value: 'speech', probability: 0.6 }] } }),
  ...
  const interpretations = {
    get: async () => ({ version: 1, activeArtifact: { payload: current } }),
    advance: async (input) => writes.push(input),
    markProjectInterpreted: async (...args) => writes.push(args),
    history: async () => [],
  };
  return { service: new AnalysisService(artifacts, objects, interpretations), writes };
}

test('user correction creates the next interpretation and relevant suggestions', async () => {
  ...
  assert.equal(result.interpretation.sourceType, 'speech'); // fake head remains unchanged; persisted bytes carry the new immutable value
...
test('correction rejects stale versions and unknown taxonomy values', async () => {
